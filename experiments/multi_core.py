...
from experiment import MultiCore, ExperimentTracker
...
NFS = [
    "nop_multicore",
    "nop_batch_lazy_multicore",
    "fw_multicore",
    "fw_batch_lazy_multicore",
    "telemetry_multicore",
    "telemetry_batch_lazy_multicore",
]
...
def build_experiment(
    ...
    if isinstance(traffic_config, PcapConfig):
        exp_name = f"{_pcap_stem(traffic_config.pcap_path)}-multi-core-{nf_name}"
    else:
        if traffic_config.traffic_dist == TrafficDist.ZIPF:
            zipf_param_str = str(traffic_config.zipf_param).replace(".", "-")
            exp_name = f"zipf-s{zipf_param_str}-f{traffic_config.nb_flows}-multi-core-{nf_name}"
        else:
            exp_name = f"uniform-f{traffic_config.nb_flows}-multi-core-{nf_name}"
...
def main():
    ...
    for nf_name, traffic_config, logical_batch_size, nb_cores in product(NFS, TRAFFIC_CONFIGS, LOGICAL_BATCH_SIZES, NB_CORES):
