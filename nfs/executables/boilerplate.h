...
static int nf_setup(int argc, char **argv) {
...
static void worker_loop_batched_straw() {
  NF_INFO("Core %u forwarding packets.", rte_lcore_id());
  ...
  while (1) {
    for (uint16_t device = 0; device < devices_count; device++) {
      ...
      packet_state_total_length(data, &(mbufs[0]->pkt_len));

      time_ns_t now       = current_time();
      uint16_t dst_device = nf_process(mbufs[0]->port, &data, mbufs[0]->pkt_len, now, mbufs[0]);
      nf_return_all_chunks(data);
